...
In order to use cookies, pass a `CookieJar` instance from
the [`tough-cookie`](https://github.com/salesforce/tough-cookie) package.
...
an [`API Gateway`](https://docs.aws.amazon.com/apigateway/api-reference/signing-requests/) with a signed request.
...
By default, Got uses the [`Link` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Link) to retrieve the
next page.\
...
Got uses the native [`http`](https://nodejs.org/api/http.html) module, which depends on the
native [`net`](https://nodejs.org/api/net.html) module.\
...
The solution is to either disable retrying (set `options.retry.limit` to `0`) or call `.persist()` on the mocked
request.
...
> - The [`proxy-agent`](https://www.npmjs.com/package/proxy-agent) family doesn't follow newest Node.js features and
	lacks support.

Although there isn't a perfect, bug-free package, [Apify](https://apify.com/)'s solution is a modern one.\
See [`got-scraping/src/agent/h1-proxy-agent.ts`](https://github.com/apify/got-scraping/blob/2ec7f9148917a6a38d6d1c8c695606767c46cce5/src/agent/h1-proxy-agent.ts)
. It has the same API as `hpagent`.

[`hpagent`](https://github.com/delvedor/hpagent) is a modern package as well. In contrast to `tunnel`, it allows keeping
the internal sockets alive to be reused.
...
Alternatively, use [`global-agent`](https://github.com/gajus/global-agent) to configure a global proxy for all
HTTP/HTTPS traffic in your program.

If you're using HTTP/2, the [`http2-wrapper`](https://github.com/szmarczak/http2-wrapper/#proxy-support) package
provides proxy support out-of-box.\
...
Got always normalizes the headers, therefore passing an `Uppercase-Header` will transform it into `uppercase-header`. To
fix this, you need to pass a wrapped agent:
...
**Note:**
> - It's a good practice to perform the validation inside the `init` hook. You can safely throw when an option is
	unknown! Internally, Got uses the [`@sindresorhus/is`](https://github.com/sindresorhus/is) package.
...
However, you can use [IPC communication](https://www.electronjs.org/docs/api/ipc-main#ipcmainhandlechannel-listener) to
